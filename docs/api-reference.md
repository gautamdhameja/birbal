...
The Birbal application defines its agent and operation-injected tool factories in `src/app/agent/run.ts` and `src/app/tools/registry.ts`. The concrete model, integration clients, logger, registry, and executor are composed only in `src/app/runtime/default.ts`.
